...
if __name__ == '__main__':
	...
	# icmc_bsi = Course("BSI", "ICMC", "cursos/BSI_BSI.txt", sbc)
	# ime = Course("BCC", "IME", "cursos/BCC_IME.txt", sbc)
	ufrgs = Course("BCC", "UFRGS", "cursos/BCC_UFRGS.txt", sbc)

	#gera um grafico
	plt.plotBarCores('bar_icmc_bcc.png', icmc_bcc.getCoresCreditsDict());
	# plt.plotBarCores('bar_ime_bcc.pdf', ime.getCoresCreditsDict());
	plt.plot2BarCores('test.png','BCC UFRGS','BCC ICMC', ufrgs.getCoresCreditsDict(),icmc_bcc.getCoresCreditsDict());
	...
	# result1 = icmc_bcc.compare(ime, 0.20)
	...
	plt.plotVenn(result2, "icmc_bccXufrgs.png");
